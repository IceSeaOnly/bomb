...
@Service
public class ClientManager extends AbastractMultiKVCacheService<DataBaseConfig, Client> {
    ...
    @Override
    protected Client load(DataBaseConfig cfg) {
        ...
        switch (type) {
            case MYSQL:
                ...
                ret = driver.init(cfg);
                ret.assertOk();
                return new MysqlClientImpl(ret.getResult(), cfg.getDbName(),cfg.getId());
            default:
